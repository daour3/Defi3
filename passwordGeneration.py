...
digits="0123456789"
#all_letters = string.ascii_letters + " .,;'-" + special_characters + digits
all_letters = string.ascii_letters + digits
...
filename = 'data/TrainEval/filterBD.txt'
filenameTrain = 'data/names/train.txt'
filenameTest = 'data/names/eval.txt'
passwd = 'data/names/foundedPassword.txt'
passwdGene = 'data/names/foundedPasswordGene.txt'
foundePass = open(passwd, "a")
foundePassGen = open(passwdGene, "r+")

"""
- Cette fonction permet de lire un fichier d'afficher la taille du fichier ainsi que son nom donné en paramètre
"""
def getLines(f):
    lines = readLines(f)
...
"""
- fonction qui prend en entrée le corpus initial et le scinde en corpus d'apprentissage et de test
"""

def split(rate, lines):
    names = []
    #for letter in string.ascii_uppercase:
    #for letter in string.ascii_letters:(modified)
...
def progressPercent(totalNames, start, names, p, samplesGenerated):
    ...
    sys.stdout.write(
        '[%s] %s%s password founded among %d samples generated (%d of %d password) on %s (goal = %.1f%% = %d password)\r' % (
...
def evaluating(decoder):
    ...
    try:
        for x in range(0,1000):
            ...
            starting_letters = ""
            taille1 = random.randint(1,5)
            for i in range(0,taille1):
                starting_letters = starting_letters + ''.join(random.choice(var) for i in range(1))
            print()
            if len(starting_letters) > 0 and len(starting_letters) < (2 * max_length):
                ...
                #print(type(mdp_eval),"\n",predicted)
                if predicted in mdp_eval:
                    #print("hello")
                    x = re.search(predicted, foundePassGen.read())
                    if not x:
                        acc = acc + 1
                        foundePassGen.write(predicted+"\n")
                print("starting letter : ",starting_letters,"predicted :",predicted)
            else:
...
if __name__ == '__main__':
    ...
    #
    parser.add_argument("-d", "--trainingData", default="data/names/test.txt", type=str,help="trainingData []")
    ...
    # repData = "data/names/Russian.txt"
